...
def create_trip(user_id, origin_city, origin_country, dest_city, dest_country,
    ...
    cursor.execute("""
        ...
        VALUES (:user_id, :origin_city, :origin_country, :dest_city, :dest_country,
                TO_DATE(:departure_date, 'YYYY-MM-DD'), TO_DATE(:arrival_date, 'YYYY-MM-DD'),
                :max_weight, :max_size, :price_per_kg, :description)
    """, {
...
def get_trips(dest_city=None, dest_country=None, min_weight=None):
    ...
    params = {}
    if dest_city:
        query += " AND UPPER(t.dest_city) = UPPER(:dest_city)"
        params["dest_city"] = dest_city
    if dest_country:
        query += " AND UPPER(t.dest_country) = UPPER(:dest_country)"
        params["dest_country"] = dest_country
    if min_weight:
        query += " AND t.max_weight >= :min_weight"
