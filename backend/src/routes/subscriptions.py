...
def update_subscription(user_id, subscription_status, subscription_id=None):
    ...
    cursor.execute("""
        ...
        WHERE id = %s
    """, (subscription_status, subscription_id, user_id))
    conn.commit()
    cursor.close()
    conn.close()
...
def create_checkout(current_user=Depends(get_current_user)):
    ...
    try:
        ...
        session = stripe.checkout.Session.create(
            ...
            mode="subscription",
            success_url="https://www.lecoindz.com/subscription-success.html?session_id={CHECKOUT_SESSION_ID}",
            cancel_url="https://www.lecoindz.com/subscription.html",
        )
...
def subscription_status(current_user=Depends(get_current_user)):
    ...
    if not user:
        raise HTTPException(status_code=404, detail="Utilisateur introuvable")
    return {"subscription_status": user["subscription_status"], "trial_end": user["trial_end"]}
...
def verify_subscription(session_id: str, current_user=Depends(get_current_user)):
    try:
        ...
        if session.status == "complete":
            update_subscription(current_user["user_id"], "ACTIVE", session.subscription)
...
def cancel_subscription(current_user=Depends(get_current_user)):
    ...
    try:
        ...
        return {"message": "Abonnement annule"}
    except stripe.error.StripeError as e:
        raise HTTPException(status_code=400, detail=str(e))